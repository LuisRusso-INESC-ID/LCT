...
{
  if(f[u].father == v) swapM(nodeT, u, v); /* Set u above and v below */
  if(f[v].father != u) return 0; /* {u,v} does not exist */
  else f[v].father = 0; /* Remove edge */
  return v;
}

void reRoot(LCT f, nodeT v)
{ /*@-fullinitblock*/
  struct LCT T = {0}; /*@=fullinitblock*/ /* Temporary struct */
  while(0 != v){
